...
We recommend the following steps for all users:
 ...
 . If you are not already on version 4.10.2, read the directions <<#upgrading_by_version,below>> to upgrade your project to 4.10.2. Stop before starting the <<#changes_5.0, upgrading from 4.10>> section.
 ...
 * The <<#rel5.0:default_memory_settings,default memory settings>> have been reduced.
 * The <<#rel5.0:default_tool_versions,default versions>> of several code quality plugins have been updated.
 * Several libraries used by Gradle have been <<#rel5.0:library_upgrades, updated>>.
...
[[upgrading_by_version]]
== Upgrading from 4.10 and earlier

Skip down to the section that applies to your current Gradle version and work your way back up through this guide until you reach 4.10.

[[changes_5.0]]
== Upgrading from 4.10

To upgrade from version 4.10 to 5.0, the following applies:
 ...
 * Please remove the `--add-modules java.xml.bind` option from `org.gradle.jvmargs`, if set. JAXB is now bundled with Java 9 and higher.
 ...
 * Gradle now bundles <<#rel5.0:jaxb_and_java9, JAXB>> for Java 9 and above.
 ...
 * With the removal of Maven 2 support, the methods to configure unique snapshot behavior have also been removed. Maven 3 only supports unique snapshots, so these methods would have had no more effect. We decided to remove them instead of leaving a deprecated no-op in place.
 * For easier configurability from statically compiled languages such as Java or Kotlin, `CheckstyleReports` and `FindbugsReports` `html` property now return `CustomizableHtmlReport`.
...
However if for some reason the dependency was later evicted from the graph, the constraint remained present. Now, when the last non-constraint edge to a dependency disappears, all constraints for that dependency will be properly removed from the graph.

[[rel5.0:default_memory_settings]]
=== [5.0] Default memory settings reduced
